...
5603E2 v.1.1	Used definitions to express most of the constants that PID requires, for ease of modification.
5603E2 v.1.2	Used definitions to express remote control inputs, for ease of modification.
...
#define LIFT_UP							btn6U					// Define lift buttons
#define LIFT_DOWN						btn6D

#define CLAW_OPEN						btn5D					// Define claw buttons
...
#define Claw_Max_Pos				(1440)				// Maximum open position of the claw
//																				// This is actually a mimimum value (in terms of pot readings)
#define Claw_Auto_Clamp			(2200)				// SensorValue(clawPot) > Claw_Auto_Clamp, claw automatically clamps
#define Claw_Auto_Open			(1800)				// Claw_Max_Pos < SensorValue(clawPot) < Claw_Auto_Open, claw automatically holds open
...
void clawCode()
{
	if((vexRT[ CLAW_OPEN ]==1) && (SensorValue[clawPot] >= Claw_Max_Pos )) //OPEN
	{
		motor[CB] = -70;
	}
	else if((vexRT[ CLAW_CLOSE ]==1))  //CLOSE
	{
		motor[CB] = 40;
	}
	...
	/**/
	else if((SensorValue(clawPot) >= Claw_Auto_Clamp ) &&	(vexRT[ CLAW_OPEN ] == 0) && (vexRT[ CLAW_CLOSE ] == 0))
		//If the claw is closed to the value the pot reads when it is clamped on a skyrise (or less),
	{
		motor[CB] = 10;
	}
	else if((SensorValue(clawPot) <= Claw_Auto_Open ) &&	(SensorValue(clawPot) >= Claw_Max_Pos ) &&	(vexRT[ CLAW_OPEN ] == 0) && (vexRT[ CLAW_CLOSE ] == 0))
