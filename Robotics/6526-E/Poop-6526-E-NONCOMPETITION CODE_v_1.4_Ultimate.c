...
task driveLeftpidStructure();
task driveleftPID();
float destinationAngleleft=0;
task ControllerPIDdrive();
float destinationAngleright = 0;
task driveRightPID();
task liftControllerPID();
float destinationAnglelift=0;
task liftControlPID();
/////////////////////////////////////////////////////////////////////////////////////////
//
//                                 Autonomous Task
...
task usercontrol()
{
	...
	startTask (controllerDrive);
	startTask (ControllerPIDdrive);
	startTask(liftControlPID);
}
...
task liftControlPID()
{
	destinationAnglelift=0;
	// start the PID task
	startTask( liftControllerPID);

	// use joystick to modify the requested position
	while( true )
	{
		// maximum change for destination angle will be 127/4*20, around 640 counts per second
		// free spinning motor is 100rmp so 1.67 rotations per second
		// 1.67 * 360 counts is 600
		if (vexRT[Btn5U]==1)
		{
			destinationAnglelift = destinationAnglelift + (vexRT[ Btn5U]*32);

		}
		if (vexRT[Btn5U]==1)
		{
			destinationAnglelift = destinationAnglelift - (vexRT[ Btn5D]*32);
...
task liftControllerPID()
{
	...
	while( true )
	{
		...
		if( pidRunning )
		{
			...
			// calculate error
			pidError = pidSensorCurrentValue - destinationAnglelift;
...
task ControllerPIDdrive()
{
	startTask(driveleftPID);
	startTask (driveRightPID);
}
...
task driveleftPID()
{
	destinationAngleleft = 0;
	// start the PID task
	startTask( driveLeftpidStructure);
	...
	while( true )
	{
		...
		destinationAngleleft = destinationAngleleft + (vexRT[ Ch3 ]/4);
...
task driveLeftpidStructure()
{
	...
	while( true )
	{
		...
		if( pidRunning )
		{
			...
			// calculate error
			pidError = pidSensorCurrentValue - destinationAngleleft;
...
task driveRightpidStructure()
{
	...
	// If we are using an encoder then clear it
	if( SensorType[ driveRight] == sensorQuadEncoder )
	...
	while( true )
	{
		...
		{
			// Read the sensor value and scale
			pidSensorCurrentValue = SensorValue[ driveRight ] * PID_SENSOR_SCALE;

			// calculate error
			pidError = pidSensorCurrentValue - destinationAngleright;
...
task driveRightPID()
{
	destinationAngleright = 0;
	// start the PID task
	startTask( driveRightpidStructure);
	...
	while( true )
	{
		...
		destinationAngleright = destinationAngleright + (vexRT[ Ch2 ]/4);
...
void pidLiftcontrol(float destinationAngle, int range1, int range2, int directionRightside, int directionLeftside, int Precision)
{
	...
	while( count!= Precision)
	{
		...
		if( pidRunning )
		{
			...
			// limit drive
			if( pidDrive > PID_DRIVE_MAX )//if the drive speed is calculated over 127, then set it to 127
				pidDrive = PID_DRIVE_MAX;
			if( pidDrive < PID_DRIVE_MIN )//if calculated speed is less than -127 set it to -127
				pidDrive = PID_DRIVE_MIN;
...
task postheights()//ONLY use when the lift is at its floor position.--Grab cube and then press the buttons
{
	while(1==1)
	{
		...
		{
			clearencoders();
			pidLiftcontrol(-310,-320,-300,-1,-1,6);//lifts the lift to the appropriate post height
			clearencoders();
		}
